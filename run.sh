...
print_header() {
    ...
    echo -e "${CYAN}  ╚════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  ${BOLD}9개의 AI 에이전트가 협업하여 소프트웨어를 개발합니다${NC}"
    echo ""
    echo -e "  ${MAGENTA}Orchestrator${NC} - 전체 조율 및 작업 분배"
    echo -e "  ${MAGENTA}Requirement → UX → Architecture → Plan → Test → Dev → Review → Docs${NC}"
    echo ""
    echo -e "  ${YELLOW}시작하기:${NC} 터미널/대시보드 실행 후 Orchestrator에게 프로젝트를 요청하세요"
    echo ""
}
...
print_menu() {
    ...
    echo -e "  ${GREEN}2)${NC} 터미널 실행 ${RED}(Auto)${NC}    완전 자동화, 최고 권한 부여"
    echo -e "  ${GREEN}3)${NC} 웹 대시보드"
    echo -e "  ${GREEN}4)${NC} 웹 대시보드 ${RED}(Auto)${NC}    완전 자동화, 최고 권한 부여"
    echo ""
    echo -e "  ${YELLOW}5)${NC} 설정                  에이전트별 AI 모델 선택"
    echo -e "  ${YELLOW}6)${NC} 세션 상태             실행 중인 에이전트 확인"
    echo ""
    echo -e "  ${BLUE}0)${NC} 종료"
    echo ""
}
